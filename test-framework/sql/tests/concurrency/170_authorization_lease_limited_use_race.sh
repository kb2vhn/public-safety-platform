...
test_file="170_authorization_lease_limited_use_race.sh"
...
work_dir="$(mktemp -d "${TMPDIR:-/tmp}/psp-authz-limited-use-race.XXXXXX")"
...
WHERE fixture.fixture_key = 'step6_limited_use_race'
\gset

UPDATE pg_temp.step4_lease_fixtures
SET lease_id = :'lease_id'::uuid
WHERE fixture_key = 'step6_limited_use_race';

SELECT sql_test.create_phase3_step4_use_decision('step6_limited_use_race') AS initial_use_decision \gset

SELECT access_control.consume_authorization_lease(
    fixture.lease_id,
    fixture.secret,
    initial.request_id,
    common.identity_id,
...
JOIN decision.decision_records AS initial
  ON initial.decision_id = :'initial_use_decision'::uuid
WHERE fixture.fixture_key = 'step6_limited_use_race';

SELECT sql_test.create_phase3_step4_use_decision('step6_limited_use_race') AS use_decision_one \gset
SELECT sql_test.create_phase3_step4_use_decision('step6_limited_use_race') AS use_decision_two \gset
...
IFS='|' read -r \
    ...
    initial_use_count initial_status \
    <<<"$(printf '%s\n' "$fixture_row" | sed -n '$p')"

if [[ -z "$lease_id" || -z "$decision_two" ]]; then
    printf 'Could not create authorization_limited_use race fixture\n' >&2
    exit 65
fi

barrier_id="${test_run_id}_authorization_limited_use_${lease_id}"
lock_key=703170
...
SELECT pg_catalog.concat_ws(
    ...
    CASE WHEN
        ...
        (SELECT pg_catalog.max(event.use_number) FROM access_control.authorization_lease_use_events AS event
         WHERE event.authorization_lease_id = lease.authorization_lease_id) = 2
        AND
        (SELECT count(DISTINCT event.use_number) FROM access_control.authorization_lease_use_events AS event
         WHERE event.authorization_lease_id = lease.authorization_lease_id) = 2
    THEN 1 ELSE 0 END,
    (SELECT count(*) FROM access_control.authorization_lease_use_events AS event
     WHERE event.authorization_lease_id = lease.authorization_lease_id
...
IFS='|' read -r final_status final_use_count consumed_timestamp event_count sequence_guard winner_event_count distinct_decisions <<<"$final_row"

precondition_guard=0
if [[ "$initial_use_count" == "1" && "$initial_status" == "ACTIVE" ]]; then
    precondition_guard=1
fi
...
psql -X --no-psqlrc --quiet --set=ON_ERROR_STOP=1 \
    ...
    --set=precondition_guard="$precondition_guard" \
    ...
    --set=sequence_guard="$sequence_guard" \
    --set=winner_event_count="$winner_event_count" \
    --set=distinct_decisions="$distinct_decisions" \
    --dbname="$test_database" <<'SQL'
SELECT sql_test.begin_file('170_authorization_lease_limited_use_race.sh');
SELECT sql_test.assert_equal_bigint('Both limited-use lease workers reached the release barrier', :'ready_count'::bigint, 2);
SELECT sql_test.assert_equal_bigint('Exactly one concurrent final limited-use slot succeeds', :'success_count'::bigint, 1);
SELECT sql_test.assert_equal_bigint('Exactly one concurrent final limited-use slot is denied', :'denied_count'::bigint, 1);
SELECT sql_test.assert_equal_bigint('Limited-use lease race has no unexpected worker or controller error', :'unexpected_count'::bigint, 0);
SELECT sql_test.assert_equal_bigint('Limited-use lease race starts with one successful use and one remaining slot', :'precondition_guard'::bigint, 1);
SELECT sql_test.assert_true('Limited-use lease race ends CONSUMED', :'final_status' = 'CONSUMED');
SELECT sql_test.assert_equal_bigint('Limited-use lease race stops exactly at its usage limit', :'final_use_count'::bigint, 2);
SELECT sql_test.assert_equal_bigint('Limited-use lease race records one consumed timestamp', :'consumed_timestamp'::bigint, 1);
SELECT sql_test.assert_equal_bigint('Limited-use lease race appends exactly two use events', :'event_count'::bigint, 2);
SELECT sql_test.assert_equal_bigint('Limited-use lease race records contiguous unique use numbers one and two', :'sequence_guard'::bigint, 1);
SELECT sql_test.assert_equal_bigint('Limited-use lease race event names the winning final-slot Decision Record', :'winner_event_count'::bigint, 1);
SELECT sql_test.assert_equal_bigint('Limited-use lease race preserves two separately attributable operations', :'distinct_decisions'::bigint, 2);
SQL

printf 'CONCURRENCY RESULT | ready=%s success=%s denied=%s unexpected=%s precondition=%s status=%s uses=%s consumed_at=%s events=%s sequence=%s winner_events=%s decisions=%s\n' \
    "$ready_count" "$success_count" "$denied_count" "$unexpected_count" \
    "$precondition_guard" "$final_status" "$final_use_count" \
    "$consumed_timestamp" "$event_count" "$sequence_guard" \
    "$winner_event_count" "$distinct_decisions"
