...
#
# Authoritative Phase 2 Step 5 validator.
#
# Default behavior:
#   - Performs complete dependency and repository preflight.
#   - Validates the accepted Step 4 boundary.
#   - Validates the full Step 5 replacement files.
#   - Runs the complete Foundation clean-install, sequential, and concurrency
#     suite.
#   - Requires the Step 5 result: 213 PASS, 0 FAIL, 3 WARN.
#
...
Default:
  Run every check required through Phase 2 Step 5, including the PostgreSQL
  clean-install, all 12 sequential tests, and all 4 concurrency tests.
...
section "Exact accepted and Step 5 file boundaries"
...
checks = [
    (
        model_path,
        "Steps 2 through 4 validated; Step 5 multi-connection concurrency "
        "proofs are an implementation candidate",
    ),
    (
        model_path,
    ...
    (
        model_path,
        "exactly one successful terminal transition, one matching terminal "
        "timestamp, one matching event, and no mixed terminal state",
    ),
    (
        migration_map_path,
        "The authoritative concurrency manifest now contains four tests.",
    ),
    (
        migration_map_path,
        "Step 6 remains responsible for the formal Phase 2 acceptance record "
        "and tag.",
    ),
]
...
if bash -n "$0"; then
    pass "Step 5 validator shell syntax is valid"
else
    fail "Step 5 validator shell syntax is invalid"
fi
...
if git diff --check -- \
    ...
    "$concurrency_establishment" \
    "$concurrency_step_up" \
    "$concurrency_terminal" >/dev/null; then
    pass "Changed Step 5 project files pass git diff --check"
else
    fail "Changed Step 5 project files contain whitespace or conflict-marker problems"
    git diff --check -- \
        "$model" \
        "$migration_map" \
        "$concurrency_manifest" \
...
if (( FAIL_COUNT > 0 )); then
    printf '\nPhase 2 Step 5 static validation FAILED.\n' >&2
    printf 'No PostgreSQL test was started.\n' >&2
    exit 1
fi

printf '\nPhase 2 Step 5 static validation PASSED.\n'
...
section "Foundation clean-install and Step 5 concurrency gate"
...
if (( FAIL_COUNT > 0 )); then
    printf '\nPhase 2 Step 5 validation FAILED.\n' >&2
    printf 'Summary: %s\n' "$summary" >&2
    exit 1
fi

printf '\nPhase 2 Step 5 validation PASSED completely.\n'
printf 'Step 5 is ready to commit before Phase 2 Step 6 acceptance work begins.\n'
