...
- Behavioral, negative, and concurrency tests as controls are implemented

The Foundation SQL has installed successfully from an empty PostgreSQL 18
database.

The accepted Phase 1 Authentication Assertion boundary completed controlled
local verification, rejection, expiration, revocation, exact-context
single-use consumption, terminal-state behavior, and a real two-connection
concurrency proof.

Its accepted test baseline is:
